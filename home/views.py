...
def upload(request):
	if request.method == "POST":
		...
		if uploadform.is_valid():
			...
			messages.info(request, "Video bylo nahráno, zpracovávám.")
...
def video(request, video_id):
	...
	editform = forms.VideoEditForm({"name": video.name, "description": video.description})
...
def post_comment(request, video_id):
	...
	if commentform.is_valid():
		...
		comment.save()
		messages.info(request, "Komentář byl přidán.")
...
def edit_video(request, video_id):
	video = get_object_or_404(models.Video, id = video_id)
	editform = forms.VideoEditForm(request.POST)
	...
	if editform.is_valid():
		...
		video.save()
		messages.info(request, "Video bylo změněno.")
		return redirect('video', video_id = video_id)
	else:
		commentform = forms.CommentForm()
...
def tags(request):
	top_tags = models.Tag.objects.annotate(num_videos = Count("video")).order_by("-num_videos")
	context = {
		"top_tags": top_tags,
	}
...
def process_video(path, video_id):
	try:
		...
		finalpath = os.path.join(settings.MEDIA_ROOT, "%d.webm" % video_id)
		subprocess.check_call(["ffmpeg", "-i", path, "-strict", "-2", "-acodec", "vorbis", "-aq", "10", "-ac", "2", "-qmax", "10", finalpath])
		os.remove(path)
